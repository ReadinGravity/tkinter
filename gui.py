...
def executor():
  ...
  if d<0:
    ...
    vysledok.config(text="nema riesenie v R")
  elif d==0:
    print(-kb/(2*ka))
    vysledok.config(text='vysledok je (x)= '-kb/(2*ka))
  else:
    print("x1= ", (-kb+d**0.5)/(2*ka)) 
    print ("x2= ",(-kb-d**0.5)/(2*ka))
    vysledok.config(text='vysledok (x1)= '+str((-kb+d**0.5)/(2*ka))+'\n'+'vysledok (x2)= '+str((-kb-d**0.5)/(2*ka)))
)
